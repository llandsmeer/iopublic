...
class DisplayNetwork:
    def __init__(self, network):
        ...
        for neuron in network['neurons']:
            ...
            for nt in neuron['traces']:
                ...
                if len(trace) >= 3:
                    vv, ff = vispy_tube.mesh_tube(trace, 1, 8)
                    faces.append(ff + sum(map(len, vertices)))
                    vertices.append(vv)
            if vertices:
                vertices = np.vstack(vertices)
                faces = np.vstack(faces)
                self.tubes.append((faces, vertices))
            else:
                self.tubes.append(([], []))
    ...
    def draw_tubes(self, view):
        meshes = []
        for ff, vv in self.tubes[::10]:
            if len(ff) == 0:
                continue
            mesh = visuals.Mesh(vv, ff, shading='smooth', color=(1.0, 0.5, 0.0))
            view.add(mesh)
            meshes.append(mesh)
        self.tube_meshes = meshes
...
def update(ev):
    ...
    for m in nw.tube_meshes:
        m.set_data(meshdata=m._meshdata, color=np.random.random(3))
